...
static mut AUDIO: *mut Box<Renderer> = null_mut();

pub struct Renderer {
    ring_buffer: Arc<Mutex<Bounded<[ceres_core::Sample; RING_BUFFER_SIZE]>>>,
    stream: cpal::Stream,
}
...
impl Renderer {
    ...
    pub fn get_mut() -> &'static mut Renderer {
        unsafe { &mut *AUDIO }
    }
