...
#### Scenario: Memory access uses parentheses

- **WHEN** an instruction accesses memory indirectly is disassembled
- **THEN** the address register is wrapped in parentheses (e.g., `"LD A, (HL)"`, `"LD (BC), A"`)

#### Scenario: High-page access format

- **WHEN** instructions $E0, $E2, $F0, $F2 are disassembled
- **THEN** they format as `"LDH ($FFxx), A"` or `"LDH A, ($FFxx)"`
