...
impl GbArea {
    pub fn new(
        ...
        let mut audio_stream = ceres_audio::Stream::new(audio_state)?;
    ...
    pub fn mut_gb(&mut self) -> MutexGuard<Gb<ceres_audio::RingBuffer>> {
        self.gb.lock().unwrap()
    }
    ...
    pub fn gb_clone(&self) -> Arc<Mutex<Gb<ceres_audio::RingBuffer>>> {
        Arc::clone(&self.gb)
    }
}
