...
vec4 get_sample(vec2 tex_coords) {
    vec4 _e3 = texture(_group_0_binding_0_fs, vec2(tex_coords));
    return _e3;
}
...
vec4 fs_scale2x(vec2 tex_coords_1) {
    vec2 dims_1 = vec2(uvec2(textureSize(_group_0_binding_0_fs, 0).xy));
    vec2 off = (vec2(1.0, 1.0) / dims_1);
    vec4 _e8 = get_sample(tex_coords_1);
    vec3 p = _e8.xyz;
    vec4 _e15 = get_sample((tex_coords_1 + vec2(0.0, -(off.y))));
    vec3 a_2 = _e15.xyz;
    vec4 _e22 = get_sample((tex_coords_1 + vec2(-(off.x), 0.0)));
    vec3 c = _e22.xyz;
    vec4 _e28 = get_sample((tex_coords_1 + vec2(off.x, 0.0)));
    vec3 b_2 = _e28.xyz;
    vec4 _e34 = get_sample((tex_coords_1 + vec2(0.0, off.y)));
    vec3 d = _e34.xyz;
    bool _e36 = eq(c, a_2);
    bool _e37 = neq(c, d);
    ...
    vec2 pp = floor((2.0 * fract((tex_coords_1 * dims_1))));
    vec3 ret_1 = ((pp.y == 0.0) ? ((pp.x == 0.0) ? p0_ : p1_) : ((pp.x == 0.0) ? p2_ : p3_));
    return vec4(ret_1, 1.0);
}

vec4 fs_scale3x(vec2 tex_coords_2) {
    ...
    vec3 p_1 = _e8.xyz;
    vec4 _e16 = get_sample((tex_coords_2 + vec2(-(off_1.x), -(off_1.y))));
    vec3 a_3 = _e16.xyz;
    vec4 _e23 = get_sample((tex_coords_2 + vec2(0.0, -(off_1.y))));
    vec3 b_3 = _e23.xyz;
    vec4 _e30 = get_sample((tex_coords_2 + vec2(off_1.x, -(off_1.y))));
    vec3 c_1 = _e30.xyz;
    vec4 _e37 = get_sample((tex_coords_2 + vec2(-(off_1.x), 0.0)));
    vec3 d_1 = _e37.xyz;
    vec4 _e43 = get_sample((tex_coords_2 + vec2(off_1.x, 0.0)));
    vec3 f = _e43.xyz;
    vec4 _e50 = get_sample((tex_coords_2 + vec2(-(off_1.x), off_1.y)));
    vec3 g = _e50.xyz;
    vec4 _e56 = get_sample((tex_coords_2 + vec2(0.0, off_1.y)));
    vec3 h = _e56.xyz;
    vec4 _e62 = get_sample((tex_coords_2 + vec2(off_1.x, off_1.y)));
    vec3 i = _e62.xyz;
    ...
    vec2 pp_1 = floor((3.0 * fract((tex_coords_2 * dims_2))));
...
vec4 get_ghost_sample(vec2 tex_coords_3) {
    vec4 _e3 = texture(_group_0_binding_2_fs, vec2(tex_coords_3));
    return _e3;
}

vec4 fs_lcd(vec2 tex_coords_4) {
    vec2 dims_3 = vec2(uvec2(textureSize(_group_0_binding_0_fs, 0).xy));
    vec2 pixel_pos = (tex_coords_4 * dims_3);
    vec2 red_offset = vec2((-0.3 / dims_3.x), 0.0);
    vec2 green_offset = vec2(0.0, 0.0);
    vec2 blue_offset = vec2((0.3 / dims_3.x), 0.0);
    vec4 _e28 = get_sample((tex_coords_4 + red_offset));
    float red_sample = _e28.x;
    vec4 _e31 = get_sample((tex_coords_4 + green_offset));
    float green_sample = _e31.y;
    vec4 _e34 = get_sample((tex_coords_4 + blue_offset));
    ...
    vec4 _e55 = get_ghost_sample(tex_coords_4);
    vec3 prev_color = _e55.xyz;
    vec3 ghosted_color = mix(final_color, prev_color, 0.5);
    return vec4(ghosted_color, 1.0);
}

vec4 fs_crt(vec2 tex_coords_5) {
    vec3 color = vec3(0.0);
    vec2 centered_coords = ((tex_coords_5 - vec2(0.5)) * 2.0);
...
vec4 desaturated(highp sampler2D txt, vec2 tex_coords_6) {
    vec4 tex = texture(txt, vec2(tex_coords_6));
