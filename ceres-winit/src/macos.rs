...
use objc::{class, runtime::Object, sel, sel_impl};
...
extern "C" fn change_shader_nearest(_: &Object, _: objc::runtime::Sel, _: id) {
    send_shader_event(ShaderOption::Nearest);
}

extern "C" fn change_shader_scale2x(_: &Object, _: objc::runtime::Sel, _: id) {
    send_shader_event(ShaderOption::Scale2x);
}

extern "C" fn change_shader_scale3x(_: &Object, _: objc::runtime::Sel, _: id) {
    send_shader_event(ShaderOption::Scale3x);
}

extern "C" fn change_shader_lcd(_: &Object, _: objc::runtime::Sel, _: id) {
    send_shader_event(ShaderOption::Lcd);
}

extern "C" fn change_shader_crt(_: &Object, _: objc::runtime::Sel, _: id) {
    send_shader_event(ShaderOption::Crt);
}

// Callback handlers for scaling menu items
extern "C" fn change_scaling_pixel_perfect(_: &Object, _: objc::runtime::Sel, _: id) {
    send_scaling_event(ScalingOption::PixelPerfect);
}

extern "C" fn change_scaling_fit_window(_: &Object, _: objc::runtime::Sel, _: id) {
    send_scaling_event(ScalingOption::FitWindow);
}

// Callback handler for open file menu item
extern "C" fn open_rom_file(_: &Object, _: objc::runtime::Sel, _: id) {
    unsafe {
        let pool = NSAutoreleasePool::new(nil);

        let open_panel = NSOpenPanel::openPanel(nil);
        open_panel.setCanChooseFiles_(cocoa::base::YES);
        open_panel.setCanChooseDirectories_(cocoa::base::NO);
        open_panel.setAllowsMultipleSelection_(cocoa::base::NO);
...
pub fn create_menu_bar() {
    unsafe {
        ...
        // let app_title = NSString::alloc(nil).init_str(crate::CERES_STYLIZED);
        let quit_title = NSString::alloc(nil).init_str("Quit");

        // cocoa::appkit::NSButton::setTitle_(app_menu_item, app_title);
        ...
        // Register the open file selector
        ...
        let _ = objc::runtime::class_addMethod(
            ...
            sel_open_file,
            std::mem::transmute_copy::<_, unsafe extern "C" fn()>(
                &(open_rom_file as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        ...
        let _ = objc::runtime::class_addMethod(
            ...
            sel_nearest,
            std::mem::transmute_copy::<_, unsafe extern "C" fn()>(
                &(change_shader_nearest as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        let _ = objc::runtime::class_addMethod(
            std::ptr::from_ref(cls).cast_mut(),
            sel_scale2x,
            std::mem::transmute_copy::<_, extern "C" fn()>(
                &(change_shader_scale2x as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        let _ = objc::runtime::class_addMethod(
            ...
            sel_scale3x,
            std::mem::transmute_copy::<_, extern "C" fn()>(
                &(change_shader_scale3x as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        let _ = objc::runtime::class_addMethod(
            ...
            sel_lcd,
            std::mem::transmute_copy::<_, extern "C" fn()>(
                &(change_shader_lcd as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        let _ = objc::runtime::class_addMethod(
            ...
            sel_crt,
            std::mem::transmute_copy::<_, extern "C" fn()>(
                &(change_shader_crt as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        ...
        let _ = objc::runtime::class_addMethod(
            ...
            sel_pixel_perfect,
            std::mem::transmute_copy::<_, extern "C" fn()>(
                &(change_scaling_pixel_perfect as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        let _ = objc::runtime::class_addMethod(
            ...
            sel_fit_window,
            std::mem::transmute_copy::<_, extern "C" fn()>(
                &(change_scaling_fit_window as extern "C" fn(_, _, _)),
            ),
            types.as_ptr().cast(),
        );
        ...
        // Add shader menu items to submenu
        add_shader_menu_item(shader_submenu, "Nearest", sel_nearest, "1");
        add_shader_menu_item(shader_submenu, "Scale2x", sel_scale2x, "2");
        add_shader_menu_item(shader_submenu, "Scale3x", sel_scale3x, "3");
        add_shader_menu_item(shader_submenu, "LCD Effect", sel_lcd, "4");
        add_shader_menu_item(shader_submenu, "CRT Effect", sel_crt, "5");

        // Set the shader submenu
        shader_submenu_item.setSubmenu_(shader_submenu);

        // Add a separator
        // view_menu.addItem_(NSMenuItem::separatorItem(nil));

        // Create scaling submenu
        ...
        add_shader_menu_item(scaling_submenu, "Pixel Perfect", sel_pixel_perfect, "p");
        add_shader_menu_item(scaling_submenu, "Fit Window", sel_fit_window, "w");

        // Set the scaling submenu
...
unsafe fn add_shader_menu_item(
    menu: id,
    title: &str,
