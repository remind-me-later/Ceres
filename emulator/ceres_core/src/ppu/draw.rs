use {
  super::{
    Ppu, LCDC_BG_AREA, LCDC_BG_B, LCDC_BG_SIGNED, LCDC_OBJL_B, LCDC_OBJ_B,
    LCDC_WIN_AREA, LCDC_WIN_B, OAM_SIZE, VRAM_SIZE,
  },
    ...
    for obj in objs.iter().take(len as usize) {
      let tile_addr = {
        ...
        let offset = if obj.attr & SPR_FLIP_Y == 0 {
          ...
          (u16::from(height) - 1)
            .wrapping_sub(u16::from(self.ly.wrapping_sub(obj.y)))
            * 2
        };
