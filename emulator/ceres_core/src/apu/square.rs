...
impl<S: super::SweepTrait> AbstractSquare<S> {
  ...
  fn set_period_half(&mut self, p_half: PHalf) { self.ltim.set_phalf(p_half); }
...
impl Square1 {
  pub(super) fn read_nr10(&self) -> u8 { self.ab.read_nrx0() }

  pub(super) const fn read_nr11(&self) -> u8 { self.ab.read_nrx1() }

  pub(super) const fn read_nr12(&self) -> u8 { self.ab.read_nrx2() }

  pub(super) fn read_nr14(&self) -> u8 { self.ab.read_nrx4() }

  pub(super) fn write_nr10(&mut self, val: u8) { self.ab.write_nrx0(val); }

  pub(super) fn write_nr11(&mut self, val: u8) { self.ab.write_nrx1(val); }

  pub(super) fn write_nr12(&mut self, val: u8) { self.ab.write_nrx2(val); }

  pub(super) fn write_nr13(&mut self, val: u8) { self.ab.write_nrx3(val); }

  pub(super) fn write_nr14(&mut self, val: u8) { self.ab.write_nrx4(val); }

  pub(super) const fn out(&self) -> u8 { self.ab.out() }

  pub(super) fn step_sample(&mut self, cycles: i32) {
    self.ab.step_sample(cycles);
  }

  pub(super) fn step_sweep(&mut self) { self.ab.step_sweep(); }

  pub(super) fn step_env(&mut self) { self.ab.step_env(); }

  pub(super) const fn true_on(&self) -> bool { self.ab.true_on() }

  pub(super) fn step_len(&mut self) { self.ab.step_len(); }

  pub(super) fn set_period_half(&mut self, p_half: PHalf) {
    self.ab.set_period_half(p_half);
  }

  pub(super) const fn on(&self) -> bool { self.ab.on() }
}
...
impl Square2 {
  pub(super) const fn read_nr21(&self) -> u8 { self.ab.read_nrx1() }

  pub(super) const fn read_nr22(&self) -> u8 { self.ab.read_nrx2() }

  pub(super) fn read_nr24(&self) -> u8 { self.ab.read_nrx4() }

  pub(super) fn write_nr21(&mut self, val: u8) { self.ab.write_nrx1(val); }

  pub(super) fn write_nr22(&mut self, val: u8) { self.ab.write_nrx2(val); }

  pub(super) fn write_nr23(&mut self, val: u8) { self.ab.write_nrx3(val); }

  pub(super) fn write_nr24(&mut self, val: u8) { self.ab.write_nrx4(val); }
