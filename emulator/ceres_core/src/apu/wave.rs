...
impl Wave {
  ...
  pub(super) const fn read_nr32(&self) -> u8 { 0x9F | (self.vol << 5) }

  pub(super) fn read_nr34(&self) -> u8 { 0xBF | self.ltim.read_on() }
  ...
  pub(super) fn write_nr31(&mut self, val: u8) { self.ltim.write_len(val); }

  pub(super) fn write_nr32(&mut self, val: u8) { self.vol = (val >> 5) & 3; }

  pub(super) fn write_nr33(&mut self, val: u8) { self.wl.write_low(val); }
  ...
  pub(super) fn step_len(&mut self) { self.ltim.step(&mut self.on); }
