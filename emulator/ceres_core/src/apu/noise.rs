...
impl Noise {
  pub(super) const fn read_nr42(&self) -> u8 { self.env.read() }

  pub(super) const fn read_nr43(&self) -> u8 { self.nr43 }

  pub(super) fn read_nr44(&self) -> u8 { 0xBF | self.ltimer.read_on() }

  pub(super) fn write_nr41(&mut self, val: u8) { self.ltimer.write_len(val); }
  ...
  pub(super) const fn out(&self) -> u8 { self.out * self.env.vol() }
  ...
  pub(super) fn step_len(&mut self) { self.ltimer.step(&mut self.on); }
