...
impl Gb {
    ...
    pub fn read_mem(&mut self, addr: u16) -> u8 {
        match addr {
            ...
            0xfe00..=0xfe9f => self.mem_tick(|gb| gb.read_oam(addr, gb.dma.is_active())),
    ...
    fn read_high(&mut self, addr: u8) -> u8 {
        match addr {
            ...
            0x40 => self.mem_tick(Gb::read_lcdc),
            0x41 => self.mem_tick(Gb::read_stat),
            0x42 => self.mem_tick(Gb::read_scy),
            0x43 => self.mem_tick(Gb::read_scx),
            0x44 => self.mem_tick(Gb::read_ly),
            0x45 => self.mem_tick(Gb::read_lyc),
            0x46 => self.mem_tick(|gb| gb.dma.read()),
            0x47 => self.mem_tick(Gb::read_bgp),
            0x48 => self.mem_tick(Gb::read_obp0),
            0x49 => self.mem_tick(Gb::read_obp1),
            0x4a => self.mem_tick(Gb::read_wy),
            0x4b => self.mem_tick(Gb::read_wx),
            0x4d if self.model == Cgb => self.mem_tick(|gb| 0x7e | gb.key1),
            0x4f if self.model == Cgb => self.mem_tick(Gb::read_vbk),
            0x55 if self.model == Cgb => self.mem_tick(|gb| gb.hdma.read_hdma5()),
            0x68 if self.model == Cgb => self.mem_tick(Gb::read_bcps),
            0x69 if self.model == Cgb => self.mem_tick(Gb::read_bcpd),
            0x6a if self.model == Cgb => self.mem_tick(Gb::read_ocps),
            0x6b if self.model == Cgb => self.mem_tick(Gb::read_ocpd),
            0x6c if self.model == Cgb => self.mem_tick(Gb::read_opri),
            0x70 if self.model == Cgb => self.mem_tick(|gb: &mut Gb| gb.read_svbk()),
    ...
    pub fn write_mem(&mut self, addr: u16, val: u8) {
        match addr {
            ...
            0xfe00..=0xfe9f => {
                self.mem_tick(|gb| gb.write_oam(addr, val, gb.dma.is_active()));
            }
    ...
    fn write_high(&mut self, addr: u8, val: u8) {
        match addr {
            ...
            0x42 => self.mem_tick(|gb| gb.write_scy(val)),
            0x43 => self.mem_tick(|gb| gb.write_scx(val)),
            0x45 => self.mem_tick(|gb| gb.write_lyc(val)),
            0x46 => self.mem_tick(|gb| gb.dma.write(val)),
            0x47 => self.mem_tick(|gb| gb.write_bgp(val)),
            0x48 => self.mem_tick(|gb| gb.write_obp0(val)),
            0x49 => self.mem_tick(|gb| gb.write_obp1(val)),
            0x4a => self.mem_tick(|gb| gb.write_wy(val)),
            0x4b => self.mem_tick(|gb| gb.write_wx(val)),
            ...
            0x4f if self.model == Cgb => self.mem_tick(|gb| gb.write_vbk(val)),
            0x50 => {
                self.tick_t_cycle();
                if val & 1 != 0 {
                    self.boot_rom.unmap();
            ...
            0x68 if self.model == Cgb => self.mem_tick(|gb| gb.write_bcps(val)),
            0x69 if self.model == Cgb => self.mem_tick(|gb| gb.write_bcpd(val)),
            0x6a if self.model == Cgb => self.mem_tick(|gb| gb.write_ocps(val)),
            0x6b if self.model == Cgb => self.mem_tick(|gb| gb.write_ocpd(val)),
            0x6c if self.model == Cgb => self.mem_tick(|gb| gb.write_opri(val)),
            0x70 if self.model == Cgb => self.mem_tick(|gb| gb.write_svbk(val)),
