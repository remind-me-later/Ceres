...
fn test_ppu_stat_interrupt_or_gate() {
    ...
    gb.ppu.write_lcdc(0x80, &mut gb.ints);
    gb.ppu.write_stat(0x60, &mut gb.ints, ceres_core::CgbMode::Dmg); // LYC=1, Mode 2=1
...
fn test_diagnostic_stat_irq_internal_line_state() {
    ...
    gb.ppu.write_stat(0x28, &mut gb.ints, ceres_core::CgbMode::Dmg); // Mode 0 + Mode 2 enabled
    let if_reg = gb.ints.read_if();
    ...
    gb.ppu.write_lyc(10, &mut gb.ints);
    gb.ppu.write_stat(0x68, &mut gb.ints, ceres_core::CgbMode::Dmg); // Mode 0 + Mode 2 + LYC enabled
    let if_reg = gb.ints.read_if();
    println!(
