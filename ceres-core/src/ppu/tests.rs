...
fn test_mooneye_oam_blocking_steady_state() {
    ...
    // Synchronize to tick 0 of line 43
    while gb.ppu.read_ly() != 43 || !matches!(gb.ppu.phase, crate::ppu::PpuPhase::OamScan(crate::ppu::OamScanStage::Running { tick: 0 })) {
        gb.ppu.tick(&mut gb.ints, crate::CgbMode::Dmg, false);
    }
    
    // Tick 0:
    assert_eq!(gb.ppu.read_oam(0xFE00), 0x00, "OAM unblocked at tick 0");
    
    // Advance 3 more ticks (total 4 ticks run: 0, 1, 2, 3)
    for _ in 0..3 {
        gb.ppu.tick(&mut gb.ints, crate::CgbMode::Dmg, false);
    }
    // Now internal tick counter is 4.
    // The logic for tick 4 runs at the START of the next tick() call.
    gb.ppu.tick(&mut gb.ints, crate::CgbMode::Dmg, false);
...
fn test_mooneye_lcdon_timing_gs_repro() {
    ...
    // We'll follow the cycles from Mooneye lcdon_timing-GS.s
    // Cycles: 0, 17, 60, 110, 130, 174, 224, 244
    // These are M-cycles. 1 M-cycle = 4 ticks (8MHz).
    ...
    // Advance 17 cycles
    for _ in 0..(17 * 4) { gb.ppu.tick(&mut gb.ints, crate::CgbMode::Dmg, false); }
    check(&gb, 17, 0x00, 0x84);

    // Advance to 60 cycles
    for _ in 0..((60 - 17) * 4) { gb.ppu.tick(&mut gb.ints, crate::CgbMode::Dmg, false); }
    check(&gb, 60, 0x00, 0x87); // Should be Mode 3

    // Advance to 110 cycles
    for _ in 0..((110 - 60) * 4) { gb.ppu.tick(&mut gb.ints, crate::CgbMode::Dmg, false); }
    check(&gb, 110, 0x00, 0x84); // Should be Mode 0 (HBlank)

    // Advance to 130 cycles
    for _ in 0..((130 - 110) * 4) { gb.ppu.tick(&mut gb.ints, crate::CgbMode::Dmg, false); }
    check(&gb, 130, 0x01, 0x82); // Should be Mode 2 of line 1 (Wait, Mooneye says 0x82? Mode 2 is bit 1)
    // 0x82 = 1000 0010 -> Mode 2, Coinc CLEAR (LY=1, LYC=0)
}
